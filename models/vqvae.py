"""
References:
- VectorQuantizer: https://github.com/CompVis/taming-transformers/blob/3ba01b241669f5ade541ce990f7650a3b8f65318/taming/modules/vqvae/quantize.py#L110
...
class VQVAE(nn.Module):
    def __init__(
        ...
        self.quantize: VectorQuantizer = VectorQuantizer(
    ...
    def idxBl_to_img_lora(self, ms_idx_Bl: List[torch.Tensor], h_Br: torch.Tensor,
                          same_shape: bool, last_one=False):
        B = ms_idx_Bl[0].shape[0]
        ms_h_BChw = []
        for idx_Bl in ms_idx_Bl:
            emb_Bld = self.quantize.embedding_lora(idx_Bl, h_Br)      # [B,l,d]
            pn = round(idx_Bl.shape[1] ** 0.5)
            ms_h_BChw.append(emb_Bld.transpose(1, 2).reshape(B, self.Cvae, pn, pn))
    ...
    def load_state_dict(self, state_dict: Dict[str, Any], strict=True, assign=False):
        ...
        if strict:
            return super().load_state_dict(state_dict=state_dict, strict=True, assign=assign)

        incompatible = super().load_state_dict(state_dict=state_dict, strict=False, assign=assign)
        missing = list(getattr(incompatible, "missing_keys", []))
        unexpected = list(getattr(incompatible, "unexpected_keys", []))

        if len(unexpected) > 0:
            msg = f"[load_state_dict][ERROR] Unexpected keys in state_dict ({len(unexpected)}):\n  {unexpected}"
            print(msg, flush=True)
            raise RuntimeError(msg)

        if len(missing) > 0:
            print(f"[load_state_dict][WARN] Missing keys ({len(missing)}) -> re-init:\n  {missing}", flush=True)

            param_map = dict(self.named_parameters(recurse=True))
            buffer_map = dict(self.named_buffers(recurse=True))

            for k in missing:
                if k in param_map:
                    p = param_map[k]
                    with torch.no_grad():
                        if p.dim() <= 1 or k.endswith("bias"):
                            p.zero_()
                        else:
                    ...
                    print(f"[load_state_dict][WARN] Missing key '{k}' not found in parameters/buffers of current module.", flush=True)

            print("[load_state_dict] Re-initialization of missing keys done.", flush=True)
