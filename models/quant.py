...
# this file only provides the VectorQuantizer used in VQVAE
...
class VectorQuantizer(nn.Module):
    ...
    def __init__(
        ...
        use_codebook_lora: bool = True, lora_rank: int = 8
    ):
        super().__init__()
        ...
        self.use_codebook_lora = use_codebook_lora
        self.lora_rank = lora_rank
        if self.use_codebook_lora:
            self.U = nn.Parameter(torch.randn(self.vocab_size, lora_rank) * 0.02)
            self.V = nn.Parameter(torch.randn(self.Cvae,      lora_rank) * 0.02)
            self.lora_gate = nn.Parameter(torch.zeros(1))
    
    @torch.no_grad()
    def _check_shapes(self, idx_Bl, h_Br):
        assert idx_Bl.dim() == 2, f"idx_Bl should be [B,l], got {idx_Bl.shape}"
        assert h_Br.dim()  == 2, f"h_Br should be [B,r], got {h_Br.shape}"
        assert h_Br.shape[1] == self.lora_rank

    def embedding_lora(self, idx_Bl: torch.Tensor, h_Br: torch.Tensor) -> torch.Tensor:
        if not self.use_codebook_lora:
            return self.embedding(idx_Bl)

        self._check_shapes(idx_Bl, h_Br)
        B, l = idx_Bl.shape
        r = self.lora_rank
        d = self.Cvae

        base = self.embedding(idx_Bl)

        U_rows = self.U[idx_Bl]
        h = h_Br.unsqueeze(1)
        offset_r = U_rows * h
        offset = offset_r @ self.V.T
        gate = torch.tanh(self.lora_gate)

        return base + gate * offset
