...
class Args(Tap):
    # data
    valid_data_path: str = 'testset'
    hr_folder: str = "FLIR-IISR/HR"
    lr_folder: str = "FLIR-IISR/LR"
    crop_size: int = 512
    lr_scale: int = 4
    
    # checkpoint
    vae_model_path: str = './checkpoints/VQVAE.pth'
    var_pretrain_path: str = "./checkpoints/VARSR.pth"
    var_test_path: str = "./checkpoints/Real-IISR.pth"
    wandb_flag: bool= 0
    exp_name: str = 'IISR'
    ...
    tblr: float = 5e-5      # base lr
    tlr: float = None       # lr = base lr * (bs / 256)
    twd: float = 0.05       # initial wd
    twde: float = 0.05         # final wd, =twde or twd
    tclip: float = 2.       # <=0 for not using grad clip
    ls: float = 0.0         # label smooth
    
    bs: int = 4           # global batch size
    batch_size: int = 4     # [automatically set; don't specify this] batch size per GPU = round(args.bs / args.ac / dist.get_world_size() / 8) * 8
    ...
    branch = safe_git("git symbolic-ref --short HEAD || git rev-parse HEAD")
    commit_id = safe_git("git rev-parse HEAD")
    commit_msg = safe_git("git log -1 --pretty=%B").splitlines()[0]
    
    acc_mean: float = None      # [automatically set; don't specify this]
    acc_tail: float = None      # [automatically set; don't specify this]
    CEL_mean: float = None        # [automatically set; don't specify this]
    CEL_tail: float = None        # [automatically set; don't specify this]
    vacc_mean: float = None     # [automatically set; don't specify this]
    vacc_tail: float = None     # [automatically set; don't specify this]
    vCEL_mean: float = None       # [automatically set; don't specify this]
    vCEL_tail: float = None       # [automatically set; don't specify this]
    vorder_loss: float = None   # [automatically set; don't specify this]
    vmse_loss: float = None     # [automatically set; don't specify this]
    vtotal_loss: float = None   # [automatically set; don't specify this]
    ...
    local_debug: bool = False#'KEVIN_LOCAL' in os.environ
    ...
    def dump_log(self):
        ...
        for k, v in {
            ...
            'CEL_mean': self.CEL_mean, 'CEL_tail': self.CEL_tail, 'acc_mean': self.acc_mean, 'acc_tail': self.acc_tail,
            'vCEL_mean': self.vCEL_mean, 'vCEL_tail': self.vCEL_tail, 'vacc_mean': self.vacc_mean, 'vacc_tail': self.vacc_tail,
...
def init_dist_and_get_args():
    ...
    if args.local_debug:
        ...
        if args.valid_data_path == '/path/to/imagenet':
            raise ValueError(f'{"*"*40}  please specify --valid_data_path=/path/to/imagenet  {"*"*40}')
    ...
    args.workers = min(max(0, args.workers), 8)
    
    args.tlr = args.ac * args.tblr
